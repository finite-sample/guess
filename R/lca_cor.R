    ...
    for (i in seq_len(n_items)) {
      est_opt[, i] <- tryCatch(
        solnp(dk_priors, guessdk_lik, eqfun = eq1dk, eqB = 1,
              LB = rep(0, 8), UB = rep(1, 8), data = transmatrix[i, ])[[1]],
    ...
    for (i in seq_len(n_items)) {
      est_opt[, i] <- tryCatch(
        solnp(nodk_priors, guess_lik, eqfun = eqn1, eqB = 1,
              LB = rep(0, 4), UB = rep(1, 4), data = transmatrix[i, ])[[1]],
...
#'   (1/4 for 4-choice items). This is the floor for gamma when difficulty → +∞.
...
#'   \item d → +∞: gamma → base_rate (hard item, random guessing)
#'   \item d → -∞: gamma → 1 (easy item, always correct even when guessing)
#' }
#'
