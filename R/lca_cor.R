...
#'
#' @title Estimate LCA model with a bounded guessing-probability link
#' @description Fits an LCA model where item difficulty is parameterized using
#'   an unbounded real-valued parameter instead of raw gamma (guessing
#'   probability). This is a reparameterized LCA, not an item-response model:
#'   it contains no person ability parameter.
#'
#' @param transmatrix Transition matrix returned from \code{\link{multi_transmat}}
...
lca_difficulty <- function(transmatrix = NULL, base_rate = 0.25,
  ...
  if (is_dk) {
    ...
    lik_fn <- make_guessdk_lik_difficulty(base_rate)
    for (i in seq_len(n_items)) {
      result <- tryCatch(
        {
          solnp(dk_priors, lik_fn,
            eqfun = eq1dk, eqB = 1,
            LB = c(rep(0, 7), -10), UB = c(rep(1, 7), 10),
            data = transmatrix[i, ], control = list(trace = 0)
          )[[1]]
        },
        error = function(e) rep(NA, 8)
      )
    ...
    lik_fn <- make_guess_lik_difficulty(base_rate)
    for (i in seq_len(n_items)) {
      result <- tryCatch(
        {
          solnp(nodk_priors, lik_fn,
            eqfun = eqn1, eqB = 1,
  ...
  class(fit) <- c("guess_difficulty_fit", class(fit))
