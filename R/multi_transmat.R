...
#' @description Needs an 'interleaved' dataframe (see interleave function).
#' Pre-test item should be followed by corresponding post-item item etc.
#' Don't knows may be coded as d/DK or, by default, NA. Function handles
#' items without don't know responses.
...
#' @param agg      Optional. Boolean. Whether or not to add a row of aggregate
#'   transitions at the end of the matrix. Default is FALSE.
...
#' @return matrix with rows = total number of items + 1 (last row contains
#'   aggregate distribution across items)
