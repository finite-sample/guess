...
alldat_dk <- data.frame(rawdata, guessdata)
names(alldat_dk)[1:nitems] <- paste0("true.t1", 1:nitems)
names(alldat_dk)[(nitems + 1):(2 * nitems)] <- paste0("true.t2", 1:nitems)
names(alldat_dk)[(2 * nitems + 1):(3 * nitems)] <- paste0("guess.t1", 1:nitems)
names(alldat_dk)[(3 * nitems + 1):(4 * nitems)] <- paste0("guess.t2", 1:nitems)

# Output data
params_dk <- cbind(diff, alpha, beta, gamma)

use_data(alldat_dk, params_dk)
