test_that("lca_cor validates inputs correctly", {
  ...
  expect_error(
    lca_cor(matrix(1:6, nrow = 2, ncol = 3)),
    "Must be element of set"
  )

  # Test invalid priors
  expect_error(
    lca_cor(matrix(1:4, nrow = 1), nodk_priors = c(0.3, 0.1, -0.1, 0.25)),
    "Element 3 is not"
  )

  expect_error(
    lca_cor(matrix(1:4, nrow = 1), nodk_priors = c(0.3, 0.1)),
    "Must have length 4"
  )
})
