test_that("lca_irt returns correct structure", {
  sim <- simulate_lca(n = 200, n_items = 2, seed = 123)
  transmatrix <- multi_transmat(sim$pre, sim$post)
  fit <- lca_irt(transmatrix)

  expect_true(inherits(fit, "guess_irt_fit"))
...
test_that("lca_irt difficulty row replaces gamma", {
...
test_that("lca_irt gamma derived correctly from difficulty", {
...
test_that("lca_irt works with DK data", {
...
test_that("lca_irt accepts custom base_rate", {
  sim <- simulate_lca(n = 200, n_items = 2, seed = 222)
  transmatrix <- multi_transmat(sim$pre, sim$post)
  fit <- lca_irt(transmatrix, base_rate = 0.2)
...
test_that("lca_irt recovers direction of difficulty ordering", {
  skip_if(
...
test_that("lca_irt recovers difficulty with reasonable accuracy", {
  skip_if(
    Sys.getenv("GUESS_FULL_TESTS") != "true",
    "Extended tests require GUESS_FULL_TESTS=true"
  )
...
test_that("lca_irt learning estimates match lca_cor approximately", {
  ...
  expect_equal(fit_irt$learning, fit_cor$learning, tolerance = 0.05)
})
...
test_that("lca_irt handles single item", {
...
test_that("lca_irt validates base_rate bounds", {
  sim <- simulate_lca(n = 100, n_items = 1, seed = 777)
  transmatrix <- multi_transmat(sim$pre, sim$post)

  expect_error(lca_irt(transmatrix, base_rate = -0.1))
  expect_error(lca_irt(transmatrix, base_rate = 1.1))
})
