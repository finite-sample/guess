test_that("multi_transmat validates inputs correctly", {
  ...
  expect_error(
    multi_transmat(pre_test_diff, pst_test_diff),
  ...
  expect_error(
    multi_transmat(pre_test_rows, pst_test_rows),
  ...
  expect_error(
    multi_transmat(pre_test, pst_test, subgroup = c(TRUE, FALSE)),
    "Must have length 3"
  )
  expect_error(
    multi_transmat(pre_test, pst_test, subgroup = c(1, 0, 1)),
...
test_that("multi_transmat works correctly without DK", {
  ...
  result <- multi_transmat(pre_test, pst_test, na_as = "missing")
...
test_that("multi_transmat handles edge cases", {
  ...
  expect_equal(ncol(result_mixed), 9)
