...
test_that("validate_compatible_dataframes works correctly", {
  ...
  expect_error(
    validate_compatible_dataframes(df1, df3),
    "Lengths of pre_test and pst_test must be the same"
  )
  expect_error(
    validate_compatible_dataframes(df1, df4),
...
test_that("validate_transition_values works correctly", {
  ...
  pst_invalid <- c("0", "1", "0")
  expect_error(
    validate_transition_values(pre_invalid, pst_invalid),
    "subset"
  )
})
...
test_that("validate_priors works correctly", {
  ...
  # Invalid inputs
  expect_error(
    validate_priors(c("0.3", "0.1"), 2, "test_priors"),
    "numeric"
  )
  expect_error(
    validate_priors(c(0.3, 0.1), 3, "test_priors"),
    "length"
  )
  expect_error(
    validate_priors(c(0.3, -0.1, 0.1), 3, "test_priors"),
    "Element 2 is not"
  )
  expect_error(
    validate_priors(c(0.3, 1.1, 0.1), 3, "test_priors"),
    "Element 2 is not"
  )
})
