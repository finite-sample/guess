...
library(devtools)
# devtools::install_github("soodoku/guess") # nolint
```
...
# MCAR
colMeans(pst_test - pre_test, na.rm = TRUE)
...
```{r eval = FALSE, count-transitions}
# Without Don't Know
pre_test_var <- c(1, 0, 0, 1, 0, 1, 0)
pst_test_var <- c(1, 0, 1, 1, 0, 1, 1)
count_transitions(pre_test_var, pst_test_var)
...
post_test <- as.data.frame(replicate(nitems, rbinom(npeople, 1, 0.6)))
names(pre_test) <- names(post_test) <- paste0("item", seq_len(nitems))

transmatrix <- count_item_transitions(pre_test, post_test)

res <- fit_item_lca_counts(transmatrix)
...
# Generate data with Don't Know responses (coded as "d")
pre_test_dk <- as.data.frame(replicate(
  nitems, sample(c("0", "1", "d"), npeople, replace = TRUE, prob = c(0.5, 0.35, 0.15))
))
post_test_dk <- as.data.frame(replicate(
  nitems, sample(c("0", "1", "d"), npeople, replace = TRUE, prob = c(0.4, 0.45, 0.15))
))
names(pre_test_dk) <- names(post_test_dk) <- paste0("item", seq_len(nitems))

transmatrix <- count_item_transitions(pre_test_dk, post_test_dk)
res_dk <- fit_item_lca_counts(transmatrix)
...
pre_test_var <- data.frame(
  pre = c(1, 0, 1, 1, 0, "d", "d", 0, 1, NA, 0, 1, 1, 1, 1, 0, 0, "d", 0, 0)
)
pst_test_var <- data.frame(pst = c(1, 0, NA, 1, "d", 1, 0, 1, 1, "d", 1, 1, 1, 0, 1, 1, 0, 1, 1, 0))
grp <- c(rep(1, 10), rep(0, 10))

group_adj(
  pre_test_var, pst_test_var,
  guessing_probability = 0, knowledge_given_dont_know = 0
)$mean_learning
group_adj(
  pre_test_var, pst_test_var,
  guessing_probability = .25, knowledge_given_dont_know = 0
)$mean_learning
stnd_cor(pre_test_var, pst_test_var, guessing_probability = .25)$learn

grp0_raw <- group_adj(
  subset(pre_test_var, grp == 0), subset(pst_test_var, grp == 0),
  guessing_probability = 0, knowledge_given_dont_know = 0
)$mean_learning
grp1_raw <- group_adj(
  subset(pre_test_var, grp == 1), subset(pst_test_var, grp == 1),
  guessing_probability = 0, knowledge_given_dont_know = 0
)$mean_learning
...
stnd_err <- sapply(diff, function(x) sqrt(var(x, na.rm = TRUE) / length(x)))
...
fit <- assess_item_lca_fit(res, pre_test, post_test)

print(fit$statistics)

fit <- assess_item_lca_fit(res_dk, pre_test_dk, post_test_dk)
...
pre_test <- as.data.frame(replicate(nitems, rbinom(npeople, 1, 0.4)))
post_test <- as.data.frame(replicate(
  nitems,
  pmin(1, pre_test + replicate(nitems, rbinom(npeople, 1, 0.25)))
))
...
# Or equivalently:
# transmatrix <- count_item_transitions(pre_test, post_test) # nolint
# fit <- fit_item_lca_counts(transmatrix) # nolint
```

##### Scores from aggregated item data

```{r eval = FALSE, score_item_lca}
transmatrix <- count_item_transitions(pre_test, post_test)
score <- score_item_lca(fit, transmatrix)
...
```{r eval = FALSE, score_individual_lca}
individual_score <- score_individual_lca(fit, pre_test, post_test)

individual_score$perplexity
hist(
  individual_score$individual_scores$perplexity,
  main = "Distribution of Individual Perplexity"
)
```
...
```{r eval = FALSE, cv_individual_lca}
cv_result <- cv_individual_lca(pre_test, post_test, k = 5, seed = 123)
...
dk_fit$params["gk", ] # Should be close to 0.15
```
