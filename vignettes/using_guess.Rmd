...
pre_test_dk <- replicate(
  nitems, sample(c(0, 1, 2), npeople, replace = TRUE, prob = c(0.5, 0.35, 0.15))
)
post_test_dk <- replicate(
  nitems, sample(c(0, 1, 2), npeople, replace = TRUE, prob = c(0.4, 0.45, 0.15))
)
...
fit <- fit_dk(
  pre_test_dk, post_test_dk,
  res_dk$params["gamma", ],
  res_dk$params[c("gg", "gk", "gd", "kk", "dg", "dk", "dd"), ],
  force9 = TRUE
)
...
# Or equivalently:
# transmatrix <- multi_transmat(pre_test, post_test) # nolint
# fit <- lca_cor(transmatrix) # nolint
```
